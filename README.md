...
- **Model otomatik geçişi:** Google, ücretsiz modelleri sık sık değiştiriyor/emekliye ayırıyor — bu proje boyunca bir model adı birden "bulunamadı" hatası vermeye başladı, başka bir seferinde de günlük ücretsiz kullanım hakkı beklenenden çok düşük çıktı. Bunun için sistem artık tek bir model adına bağımlı değil; `backend/api.js` içindeki `MODEL_CANDIDATES` listesinde sırayla denenecek modeller tutulur (şu an: önce `gemini-2.5-flash`, o çalışmazsa `gemini-3.5-flash`). Bir model "bulunamadı" hatası ya da **günlük** kota hatası verirse sistem otomatik olarak listedeki bir sonraki modele geçer — elle müdahaleye gerek kalmaz
- **"RESOURCE_EXHAUSTED" / "quota exceeded" hatası (günlük limit):** Bu, dakikalık değil **günlük** bir sınırdır — o an denenen modelin ücretsiz katmanda bir günde izin verdiği toplam istek sayısı dolmuş demektir. Sistem bu durumda zaten otomatik olarak bir sonraki modele geçmeyi dener (yukarıya bak); TÜM modellerin günlük kotası aynı anda dolarsa: (a) ertesi gün kotalar sıfırlanınca sorun kendiliğinden düzelir, (b) aynı Gemini anahtarını başka bir uygulama/denemede de kullanıyorsan kotayı paylaşıyor olabilirsiniz, (c) katılımcı sayısı artacaksa https://aistudio.google.com/apikey üzerinden faturalandırmalı (ücretli) bir plana geçmek ya da farklı bir Gemini anahtarı kullanmak gerekebilir
- **Not:** `gemini-2.5-flash`, Google tarafından "deprecated" (kullanımdan kaldırılıyor) olarak işaretlenmiş durumda ve 16 Ekim 2026'da tamamen kapatılması planlanıyor. O tarihe kadar sorunsuz çalışmaya devam edecek, ama tarih yaklaştıkça `MODEL_CANDIDATES` listesine güncel bir model adı eklemek (ya da sıralamayı değiştirmek) gerekecek
