...
🧠 Nasıl Çalışır (Akış)
Admin ve katılımcılar Google ile giriş yapar. Admin panelinde kimlerin bağlı olduğu canlı görünür.
Admin "Yapay Zekadan Yeni Vaka İste ve Yarışmayı Başlat" butonuna basar.
...
2. Firebase Setup
Firebase Console → projeni aç
Firestore Database oluştur (zaten yoksa)
Authentication → Sign-in method → Google etkinleştir
Authentication → Settings → Authorized domains kısmına Railway domain'ini ekle (örn. `nietzsche-yarismasi-production.up.railway.app`)
Proje Ayarları (⚙️) → Service Accounts → Generate New Private Key — bu indirdiğin JSON dosyasından `project_id`, `client_email`, `private_key` değerlerine ihtiyacın olacak (backend'in Firestore'a admin olarak yazabilmesi için)
Firestore Database → Rules kısmına bu repodaki `firestore.rules` dosyasının tam içeriğini yapıştır ve Publish'e bas (Eğitimler özelliği için `trainings` koleksiyonu kuralı eklendi — eskisinin üzerine tamamını yaz)
...
Filozofun Bakış Açısına Sadakat (0-30 puan)
Mantıksal Tutarlılık (0-20 puan)
Vakaya Uygunluk (0-20 puan)
İkna Gücü ve Retorik Yetkinlik (0-15 puan)
Kurumsal Uygulanabilirlik (0-15 puan)
🏆 Ödüllendirme
